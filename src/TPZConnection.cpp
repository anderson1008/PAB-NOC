...
void TPZConnection :: run(uTIME time)
{
   if( m_InputPort && m_OutputPort )
   {
      ...
      m_InputPort->setPG(m_OutputPort->getPG(PG),PG); // By Anderson: transmit each cycle.
      m_InputPort->setPG(m_OutputPort->getPG(WU),WU); // By Anderson: transmit each cycle.
      m_InputPort->setPG(m_OutputPort->getPG(ACK),ACK); // By Anderson: transmit each cycle.
		m_InputPort->updatePG(); // By Anderson
