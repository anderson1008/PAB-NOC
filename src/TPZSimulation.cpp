   ...
   buffer += TPZString(" Traffic Pattern         = ") + m_TrafficPattern->asString() +
	     ...
	     //****************************************************************************************************************************
             ...
             if( m_verbosity>2 )
             {
                ...
                buffer += TPZString("\n****************** LINK USAGE MAP (%) *******************");
                buffer += TPZString("\n");
                for (int l=0; l<m_Network->getSizeZ(); l++) //A: Z dimension
                {
	               buffer += TPZString("\n************************** Z=") + TPZString(l) +TPZString(" **************************");
		           buffer += TPZString("\n");
                   for (int k=0; k<m_Network->getSizeY(); k++)
                   {
                      for (int j=0; j<m_Network->getSizeX(); j++)
                      {
                         // A: extract link utilization value.
...
unsigned long maxInject; // CHIPPER Support
//*************************************************************************
//:
                                     ...
                                     INDEX& index)
{
   ...
   TPZString notClosed=tag->tagName(); // A: as long as a tag is not closed, check every tag inside it.
   ...
   while( nextTag && nextTag->tagName()!=endTag )
   {
      ...
      else if( tagName == TPZ_TAG_STOPSIMUL_MSG )
      {
         ...
         maxInject = stopSimulationMsg.asInteger();
         m_isRafagaMode=true;
      }
