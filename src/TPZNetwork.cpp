...
//*************************************************************************
//  f: void increHeadTailCount ();
//
//  d:
//:
//*************************************************************************

void TPZNetwork::increHeadTailCount () {
...
unsigned TPZNetwork::getHeadTailCount (){
   return m_num_head_tail;
}
...
void TPZNetwork::postInitialize() {
    ...
    for (register int i=0; i<m_SizeX; i++)
		...
		#ifdef PTOPAZ
                int linearindex=i+j*m_num_head_tailSizeX+k*m_SizeX*m_SizeY;
...
Boolean TPZNetwork::sendMessage(TPZMessage* msg) {
    ...
    TPZMessage::assignUniqueId(msg); // CHIPPER Support
