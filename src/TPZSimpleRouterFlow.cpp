...
void TPZSimpleRouterFlow :: initialize()
{
   ...
   m_vnets=((TPZSimpleRouter&)getComponent()).getVnets(); // A: TPZConst::vnets is the number of virtual network.
...
Boolean TPZSimpleRouterFlow :: inputReading()
{
   ...
   for( outPort = 1; outPort <= m_ports; outPort++)
   {
      ...
      // A: check if the connection associated with outPort has been established.
      //    if not, go and check the next one.
      if( ! (inPort = m_connections[outPort] ) ) //A: = is probably overloaded in TPZVector.hpp
      {
                 continue;
      }

      // A: possibly to detect if packet is fragmented at the link
      ...
      if(outPort == m_ports) virtualChannel=1; // if #_of_input == #_of_output ---> VC = 1
      ...
      if( ! ( m_routingtime[inPort]-- ) )
      {
	     ...
	     // Uni-cast Message Start Here.
	     else
	     {
	         m_routing[inPort] = 0; // A: why 0? I guess it means the msg is expired.
	                                // m_routing is TPZMessage type.
	     }


         //A: possibly, send "mess" to the "vitualChannel" of "outPort"
         // or read "mess" from "outPort" to "vitualChannel"
	     outputInterfaz(outPort)->sendData(mess,virtualChannel);
         //A: increment event vount based on the type of event.
         //   Event is a enum type defined in TPZNetwork.hpp.
         ...
         {
            m_connections[outPort] = 0; // A: if the flit is the tail flit, release the connection.
            #ifndef NO_TRAZA
   ...
   //******************************************************************************************************************
   // A: I may need to change it to give priority to each flit.
   ...
   for(i = 0; i < m_ports*m_vnets ; i++, inPort = inPort - 1)
   {
      ...
      // A: another words, only head flit will continue.
      ...
      // A: read the STOP signal on virtualChannel
      // NOT really clear!
      if( outputInterfaz(outPort)->isStopActive(virtualChannel) )
      {
         if( ! m_token ) m_token = inPort; // A: token is assigned here.
         continue;
      }

      // A: if an output is available
   ...
   // No ports have been rejected or satisfy the condition.
   ...
   for(inPort = 1; inPort <= m_ports*m_vnets; inPort++)
   {
      ...
      // A: I think it means if the routing is empty
      if( ! m_routing[inPort] )
      {
        // Take the next message from the corresponding fifo
         if(m_fifos[inPort].numberOfElements()!=0)
         {
            ...
            if( m_routing[inPort]->isHeader() || m_routing[inPort]->isHeadTail() )
            {
                ...
                updateMessageInfo(m_routing[inPort]);
                m_routingtime[inPort] = getHeaderDelay(); // A: why set header delay? maybe only need for t
            }
            else
            {
...
//
//  d:
//: A: update the msg header. I think look-ahead routing is implemented here.
//*************************************************************************
...
{
   TPZMessage* msg;
   inputInterfaz(interfaz)->getData(&msg,cv); // A: construct a msg when channel is ready.
   ...
   m_sync[m_ports*(cv-1)+interfaz]=msg; // A: put msg on a carrier to be sent to the next hop.
   return true;
}
