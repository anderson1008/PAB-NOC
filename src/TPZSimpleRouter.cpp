...
// A: the functional behavior is selected here.
void TPZSimpleRouter :: buildFlowControl()
{
   ...
   if(getFlowControl()) // A: class in TPZRunnableComponent
   {
      getFlowControl() -> initialize(); // A: I guess it calls TPZFlow::initialize()
   }
   else
   {
      TPZString err;
...
//                                  TPZComponent * owner,
//                                  INDEX & index);
//  d: (By Anderson) Passing the tag value.
//:
//*************************************************************************
                                     ...
                                     INDEX& index)
{
   ...
   // if the input tag is provided, pass the value accordingly.
   // if either one of the tag doesn't exist, the program will be terminated.
   ...
   // A:
   // default value is 0.
   // use default value if it is not specified in input.
   ...
   // passing routing control information.
   ...
   //A: passing buffer control information.
   TPZString bufferControlString = ((TPZRouter*)owner)->valueForBufferControl();
   //A: no buffer control - BLESS by default
   ...
   // A: create simpleRouter obj based on the tag value just obtained.
   ...
   // A: I don't understand this line.
   //    looks like to retrieve the global data.
   TPZGlobalData& gData = ((TPZSimulation*)(owner->getSimulation()))->globalData((TPZNetwork*)(owner->getOwner()));

   TPZString endTag = TPZString("/") + tag->tagName(); // check if tag ends.
   TPZTag* nextTag = TPZComponent::routerBuilder->getTagWithIndex(++index);

   // A: set input and output ports type
   ...
   // A: all of tag will be read in here.
   // Check if port is half-duplex or whatever.
