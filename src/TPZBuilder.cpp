...
TPZBuilder :: TPZBuilder(const TPZString& sgmlFile)
            : m_FileName(sgmlFile),
              m_TagArray(0)
{
   m_SgmlFile = new ifstream(sgmlFile);  // Anderson: ifstream - stream class to read from files.
   if( ! m_SgmlFile->good() ) // Anderson: Check whether the state of stream is good.
   {
      TPZString err;
